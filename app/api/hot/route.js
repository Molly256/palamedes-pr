...
export async function GET(request) {
  try {
    ...
    const historyKey = `tx:${phone}:history`; // FIXED: Reads from the exact history key

    const [userProfile, rawHistory] = await Promise.all([
      redis.hgetall(rootUserKey),
      redis.lrange(historyKey, 0, -1) // FIXED: Pulls from history key only
    ]);
...
export async function POST(request) {
  try {
    ...
    const rootUserKey = `user:${phone}`;
    const historyKey = `tx:${phone}:history`; // FIXED: This is the ONLY list key used now
    ...
    if (action === 'BUY_HOT' || action === 'BUY_SHARES') {
      ...
      const jsonTxString = JSON.stringify(txItem);

      await Promise.all([
        redis.hset(rootUserKey, { availableBalance: String(wallet) }),
        redis.lpush(historyKey, jsonTxString) // FIXED: Pushes ONLY to history key
      ]);

      const freshHistory = await redis.lrange(historyKey, 0, -1);
    ...
    if (action === 'COLLECT_HOT') {
      ...
      await redis.del(historyKey); // FIXED: Clears the history key before rewriting

      await Promise.all([
        redis.hset(rootUserKey, { availableBalance: String(wallet) }),
        redis.rpush(historyKey, ...updatedHistoryArray.reverse()), // FIXED: Rewrites cleanly to history key
        redis.lpush(historyKey, jsonTxString) // FIXED: Pushes final collection status strictly to history key
      ]);
