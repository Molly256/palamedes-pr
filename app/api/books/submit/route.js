...
export async function POST(req) {
  try {
    const { phone, bookId, action } = await req.json() // <- REMOVED idempotencyKey
    if (!phone || !bookId || !action) { // <- REMOVED idempotencyKey check
    ...
    // REMOVED: idemKey, completedSetKey
    ...
    if (action === 'submit') {
      ...
      // REMOVED: redis.sadd(completedSetKey, bookIdStr) check
      ...
      const tx = {
        id: `${phone}:${today}:${bookIdStr}:${Date.now()}`, // <- random id, no idempotencyKey
      ...
      // REMOVED: redis.smembers(completedSetKey)
      ...
      return NextResponse.json({
        ...
        user: {
          ...updatedUser,
          completedBooks: [], // <- empty array, frontend will use status: 'submitted'
